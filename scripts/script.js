...
function setTheme(theme) {
  ...
  const kind = window.matchMedia?.("(prefers-color-scheme: dark)").matches ? "dark" : "light"
  if (theme == 'light' || (theme == 'auto' && kind == 'light')) {
      document.getElementById('__rec').style.visibility = "hidden";
  } else {
      document.getElementById('__rec').style.visibility = "";
  }
}
